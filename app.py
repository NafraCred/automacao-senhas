...
def navigate_via_proxysite(driver, target_url, timeout=NAV_TIMEOUT):
    ...
    # 2) Primeiro: acessar explicitamente proxysite.pt e fechar anúncio se aparecer,
    #    inserir a URL do gerador no campo "Enter Url" (name="d") e clicar "Ir".
    try:
        PROXY_PT = "https://www.proxysite.com/pt/"
        driver.get(PROXY_PT)
        time.sleep(1.0)

        # tentar fechar o anúncio se o botão existir (id="dismiss-button")
        try:
            # procurar pelo id dismiss-button e clicar via JS para evitar interceptações
            dismiss = None
            try:
            ...
            if dismiss:
                try:
                    ...
                    try:
                        ActionChains(driver).move_to_element(dismiss).click(dismiss).perform()
                        time.sleep(0.4)
                    except Exception:
                        pass
        except Exception:
        ...
        # localizar o campo de URL (name="d" ou placeholder "Enter Url") e preencher com target_url
        input_el = None
        try:
            els = driver.find_elements(By.NAME, "d")
            if els:
                input_el = els[0]
        except Exception:
            input_el = None
        ...
        if not input_el:
            try:
                els = driver.find_elements(By.CSS_SELECTOR, "input[placeholder='Enter Url'], input[placeholder='Enter Url ']")
                if els:
                    input_el = els[0]
            except Exception:
                input_el = None
        ...
        if input_el:
            ...
            # encontrar botão "Ir" do proxysite e clicar
            btn = None
            try:
                # botão type=submit com texto "Ir"
                candidates = driver.find_elements(By.XPATH, "//button[@type='submit' and (normalize-space(.)='Ir' or normalize-space(.)='IR' or normalize-space(.)='ir')]")
                if candidates:
                    btn = candidates[0]
            except Exception:
                btn = None
            if not btn:
                try:
                    # fallback: primeiro botão submit na página
                    candidates = driver.find_elements(By.XPATH, "//button[@type='submit']")
                    if candidates:
                        btn = candidates[0]
                except Exception:
                    btn = None
            ...
            if btn:
                ...
                # aguardar conteúdo proxied aparecer
                ...
                while time.time() < end:
                    ...
                    try:
                        ...
                        pass
                # se não encontrou, continuar para fallback proxysites
            else:
                # se não encontrou botão, tentar navegar diretamente via JS
                try:
        ...
        # se qualquer erro aqui, vamos tentar os proxysites alternativos abaixo
        pass

    # 3) Lista de proxysites públicos como fallback
    PROXY_URLS = [
        "https://www.proxysite.com/pt/",
    ...
    for PROXY_URL in PROXY_URLS:
        try:
            ...
            # possíveis seletores para o campo de URL
            candidates = [
                ("name", "d"), ("id", "d"),
                ("name", "u"), ("id", "u"),
                ("name", "url"), ("id", "url"),
            ...
            for by, val in candidates:
                try:
                    if by == "name":
                        els = driver.find_elements(By.NAME, val)
                    elif by == "id":
                        els = driver.find_elements(By.ID, val)
                    else:
                        els = driver.find_elements(By.CSS_SELECTOR, val)
                    if els:
                        input_el = els[0]
                        break
                except Exception:
            ...
            if input_el:
                ...
                # tentar submeter o form de forma robusta
                try:
                ...
                # se não há campo, forçar navegação via JS
                try:
                    driver.execute_script("window.location.href = arguments[0];", target_url)
                except Exception:
                    pass
    ...
    save_capture("proxysite", "timeout_or_no_input", last_html or "")
    ...
    try:
        ...
        for step in range(MAX_QUESTION_STEPS):
            ...
            if not resposta_valor:
                ...
                try:
                    save_capture(session_id, f"unmatched_question_step{step}", cur_html)
                except Exception:
                    pass
                try:
                    driver.quit()
                    ...
                    pass
                SESSIONS.pop(session_id, None)
                return render_template_string(TEMPLATE, captcha_data=None, result=f"Não foi possível determinar a resposta para a pergunta:\n\n{pergunta}\n\nTrecho da página:\n{snippet}", session_id=None, cpf=None, mat=None)

            # agora temos resposta_valor: tentar selecionar a opção correspondente na página
            try:
                # se resposta_valor corresponde ao value do radio, clicar nele
                selected = None
                try:
                    radios = driver.find_elements(By.XPATH, "//input[@type='radio' and @name='resposta']")
                    for r in radios:
                        try:
                            v = r.get_attribute("value") or ""
                            if v.strip() == resposta_valor.strip():
                ...
                # se não selecionou por value, tentar selecionar pelo texto do label
                if not selected and options:
                    for val, text in options:
                        txt_norm = re.sub(r'[^0-9A-Za-zÀ-ÿ\s]', ' ', (text or "")).strip().lower()
                        if txt_norm and (txt_norm in resposta_valor.strip().lower() or resposta_valor.strip().lower() in txt_norm):
                            # clicar no radio com this value
                            try:
                                r_el = driver.find_element(By.XPATH, f"//input[@type='radio' and (@value='{val}' or normalize-space(@value)='{val}')]")
                                driver.execute_script("arguments[0].click();", r_el)
                                selected = True
                                break
                            except Exception:
                                continue

                # se ainda não selecionou, tentar preencher um campo de texto com a resposta (caso a pergunta peça texto)
                if not selected:
                    try:
                        txt_inputs = driver.find_elements(By.XPATH, "//input[@type='text' or @type='textarea' or @name='resposta_text']")
                        for ti in txt_inputs:
                            try:
                                driver.execute_script("arguments[0].value = arguments[1];", ti, resposta_valor)
                                selected = True
                                break
                            except Exception:
                                continue
                ...
                # aguardar nova página / resultado
                end = time.time() + POLL_AFTER_CLICK
                while time.time() < end:
                    time.sleep(POLL_INTERVAL)
                    cur_html = driver.page_source
                    senha = extract_senha_from_html(cur_html)
                ...
                try:
                    save_capture(session_id, f"error_step_{step}", driver.page_source)
                except Exception:
                    pass
                try:
                    driver.quit()
                    ...
                    pass
                SESSIONS.pop(session_id, None)
                return render_template_string(TEMPLATE, captcha_data=None, result=f"Erro ao tentar responder pergunta: {e}", session_id=None, cpf=None, mat=None)

        # se saiu do loop sem senha
        try:
            save_capture(session_id, "no_password_after_steps", driver.page_source)
        except Exception:
            pass
        try:
            driver.quit()
            ...
            pass
        SESSIONS.pop(session_id, None)
        return render_template_string(TEMPLATE, captcha_data=None, result="Não foi possível obter a senha após as tentativas automáticas. Verifique as capturas.", session_id=None, cpf=None, mat=None)
    except Exception as e:
        try:
            save_capture(session_id, "exception_submit", driver.page_source)
        except Exception:
            pass
        try:
            driver.quit()
            ...
            pass
        SESSIONS.pop(session_id, None)
        return render_template_string(TEMPLATE, captcha_data=None, result=f"Erro inesperado: {e}", session_id=None, cpf=None, mat=None)
...
def download_password(token):
    ...
    resp.headers["Content-Disposition"] = f"attachment; filename=senha_{int(time.time())}.txt"
