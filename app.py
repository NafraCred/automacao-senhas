...
def start_selenium_headless(proxy=None):
    ...
    # ensure window size (some environments need explicit call)
    try:
...
# --- novo helper: navegar via proxyium ---
def navigate_via_proxyium(driver, target_url, timeout=NAV_TIMEOUT):
    """
    Abre https://proxyium.com/, insere target_url no campo com id 'unique-form-control',
    clica no botão 'unique-btn-blue' e aguarda até que o conteúdo proxied carregue
    (detectando o campo 'cpf' ou 'matricula'). Lança RuntimeError em caso de timeout.
    """
    ...
    try:
        ...
        raise RuntimeError(f"Falha ao abrir proxyium: {e}")
    ...
    while time.time() < end:
        try:
            try:
                input_el = driver.find_element(By.ID, "unique-form-control")
            except Exception:
                input_el = None
            try:
                btn_el = driver.find_element(By.ID, "unique-btn-blue")
            except Exception:
                btn_el = None
            if input_el and btn_el:
                break
        except Exception:
            pass
        time.sleep(0.4)

    if not input_el or not btn_el:
        # tentar localizar por name/placeholder como fallback
        try:
            els = driver.find_elements(By.CSS_SELECTOR, "input[placeholder*='Put a URL']") or driver.find_elements(By.NAME, "url")
            if els:
                input_el = els[0]
        except Exception:
            input_el = input_el
        try:
            if not btn_el:
                btns = driver.find_elements(By.CSS_SELECTOR, "button.btn-blue") or driver.find_elements(By.XPATH, "//button[contains(., 'GO')]")
                if btns:
                    btn_el = btns[0]
        except Exception:
            btn_el = btn_el
    ...
    if not input_el or not btn_el:
        raise RuntimeError("Campo de URL ou botão GO não encontrados no proxyium.")

    # tentar fechar banners/modais comuns antes de preencher
    try:
        # seletores comuns; alguns podem não existir, por isso try/except
        modal_selectors = [
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError("Falha ao clicar no botão GO do proxyium.")
    except Exception as e:
        raise RuntimeError(f"Erro ao preencher/clicar no proxyium: {e}")

    # aguardar e trocar para nova aba se o proxied content abriu em outra janela
    time.sleep(0.6)
    try:
    ...
    # aguardar o proxied content carregar: procurar campo cpf/matricula usando busca robusta em frames
    end2 = time.time() + timeout
    while time.time() < end2:
        try:
            # tenta encontrar o campo cpf em qualquer frame/janela usando a função robusta se disponível
            try:
                el = find_element_across_frames(driver, By.ID, "cpf", timeout=2)
            except Exception:
                el = None
            if not el:
                try:
                    el = find_element_across_frames(driver, By.NAME, "cpf", timeout=2)
                except Exception:
                    el = None
            if not el:
                try:
                    el = find_element_across_frames(driver, By.ID, "login", timeout=2)
                except Exception:
                    el = None
            if not el:
                try:
                    el = find_element_across_frames(driver, By.ID, "matricula", timeout=2)
                except Exception:
                    el = None
            if el:
                return driver
    ...
    try:
        save_capture("proxyium", "timeout_no_cpf", driver.page_source)
    except Exception:
        pass
    raise RuntimeError("Timeout aguardando conteúdo proxied via proxyium. Verifique captures.")
...
def navigate_via_proxysite(driver, target_url, timeout=NAV_TIMEOUT):
    ...
    # 2) Lista de proxysites públicos como fallback
    PROXY_URLS = [
...
def start_headless():
    ...
    try:
        # primeiro tente usar proxyium (abre proxyium, insere URL e clica GO)
        try:
            navigate_via_proxyium(driver, URL_BASE, timeout=NAV_TIMEOUT)
        except Exception as e_proxyium:
            # se falhar, cai para o proxysite fallback existente
            try:
                navigate_via_proxysite(driver, URL_BASE, timeout=NAV_TIMEOUT)
            except Exception as e_proxysite:
                try:
                    driver.quit()
                except:
                    pass
                return f"Erro ao abrir via proxyium/proxysite: {e_proxyium} ; {e_proxysite}", 500
