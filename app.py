...
def navigate_via_proxyium(driver, target_url, timeout=NAV_TIMEOUT):
    ...
    # aguardar o proxied content carregar: procurar campo cpf/matricula
    end2 = time.time() + timeout
    while time.time() < end2:
        try:
            # verificar se o conteúdo proxied já expõe o campo cpf (pode estar em iframe)
            if driver.find_elements(By.ID, "cpf") or driver.find_elements(By.NAME, "cpf") or driver.find_elements(By.ID, "login") or driver.find_elements(By.ID, "matricula"):
                return driver
        except Exception:
            pass
        time.sleep(0.6)
